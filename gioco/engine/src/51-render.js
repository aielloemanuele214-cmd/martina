...
// Mirroring intelligente: se una direzione orizzontale non ha foglio dedicato,
// si riflette l'altra (left = right ribaltato). Front/back restano sempre
// dedicati (non riflettibili). Così bastano 3 fogli (down/up/right): il
// personaggio si orienta sempre verso il punto toccato, senza "camminare
// all'indietro" e senza sprite duplicati.
function playerFrame(){
  const P=SPR.player;
  let dir=player.dir, flip=false, key=P.fogli[dir];
  if(!key){
    if(dir==='left'  && P.fogli.right){ key=P.fogli.right; flip=true; }
    else if(dir==='right' && P.fogli.left){ key=P.fogli.left; flip=true; }
    else key=P.fogli.down || P.fogli[Object.keys(P.fogli)[0]];   // fallback sicuro
  }
  const st=P.stati[player.state]||P.stati.idle;
  if(st.seq){
    const i=Math.floor(player.animT/st.dur)%st.seq.length;
    return {key, frame:st.seq[i], dy:0, flip};
  }
  ...
  return {key, frame:st.frame||0, dy, flip};
}
...
function render(){
  ...
  if(!scena){
    ...
    ents.push({y:player.y, draw(){
      ...
      blit(pf.key, pf.frame, player.x, player.y, pf.flip, a, pf.dy);
    }});
  }
